...
**Control Colors** (視認性重視 - 改善版2):
- Slider track: `CColor(30, 30, 30, 255)` - #1E1E1E (Very dark, much darker than background)
- Slider handle: `CColor(255, 255, 255, 255)` - #FFFFFF (White for maximum contrast)
- Slider frame: `CColor(20, 20, 20, 255)` - #141414 (Almost black border)
- Knob handle: `CColor(220, 220, 220, 255)` - #DCDCDC (Very light gray handle, high visibility)
- Knob corona: `CColor(160, 160, 160, 255)` - #A0A0A0 (Light gray arc, clearly visible)
...
**Design Rationale**:
- スライダートラックをより暗く(30,30,30)にして背景(80,80,80)との境界をさらに明確化
- スライダーハンドルを白(255,255,255)で最大コントラストを確保
- スライダーフレームをほぼ黒(20,20,20)にしてトラック領域を強調
- ノブハンドルをより明るく(220,220,220)にして現在位置を明確化
- ノブコロナを明るく(160,160,160)にして背景との差を大きくし視認性向上
...
slider->setMin(0.0f);
slider->setMax(1.0f);
slider->setBackColor(CColor(30, 30, 30, 255));    // Track: Very dark (much darker)
slider->setFrameColor(CColor(20, 20, 20, 255));   // Border: Almost black
slider->setValueColor(CColor(255, 255, 255, 255)); // Handle: White (maximum contrast)
```

**Gain/Delay Knobs** (視認性改善版2):
```cpp
...
knob->setColorHandle(CColor(220, 220, 220, 255));  // Very light gray handle (higher visibility)
knob->setCoronaColor(CColor(160, 160, 160, 255));  // Light gray corona (brighter)
