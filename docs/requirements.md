...
- [x] **視認性要件（重要 - 改善版2）**:
  - [x] スライダーのトラック領域が背景と明確に区別できる（背景80,80,80より大幅に暗い30,30,30）
  - [x] スライダーの現在位置が明確に視認できる（白255,255,255で最大コントラスト）
  - [x] スライダーフレームが十分に暗く、トラック領域を強調（20,20,20）
  - [x] ノブが背景と明確に区別できる（コロナ160,160,160で背景より明るく）
  - [x] ノブのハンドルが現在位置を明確に示す（220,220,220で高視認性）
  - [x] すべてのコントロールが暗い環境でも視認可能
